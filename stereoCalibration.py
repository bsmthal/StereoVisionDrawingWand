...
ret, mtxL, distL, rvecsL, tvecsL = cv.calibrateCamera(objpointsl, imgpointsL, gray.shape[::-1], None, None)
ret, mtxR, distR, rvecsR, tvecsR = cv.calibrateCamera(objpointsr, imgpointsR, gray.shape[::-1], None, None)
...
for framer, framel in zip(stereo_right_images,stereo_left_images):
    ...
    if retR == True and retL == True:
        ...
        cv.waitKey(0)
cv.destroyAllWindows
retStereo, newCameraMatrixL, distL, newCameraMatrixR, distR, rot, trans, essentialMatrix, fundamentalMatrix = cv.stereoCalibrate(objpoints, imgpointsL, imgpointsR, mtxL, distL, mtxR, distR, gray.shape[::-1], criteria_stereo, flags)
